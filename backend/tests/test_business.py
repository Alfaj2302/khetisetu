...
def test_forecast_is_empty_on_a_fresh_seed(client):
    resp = client.get(f"{API}/business/forecast", params={"district_id": 1})
    assert resp.status_code == 200
    assert resp.json() == []
...
def test_forecast_filters_by_district_product_and_year(client, db_conn):
    ...
    other_district = client.get(f"{API}/business/forecast", params={"district_id": 2}).json()
    assert len(other_district) == 1

    no_filter = client.get(f"{API}/business/forecast").json()
...
def test_alerts_is_empty_on_a_fresh_seed(client):
    resp = client.get(f"{API}/business/alerts")
    assert resp.status_code == 200
    assert resp.json() == []
...
def test_alerts_excludes_monitor_action_and_respects_district_filter(client, db_conn):
    ...
    district_1 = client.get(f"{API}/business/alerts", params={"district_id": 1}).json()
    assert len(district_1) == 1
    assert district_1[0]["severity"] == "High"
    assert district_1[0]["message"] == "Urea shortage expected"

    district_2 = client.get(f"{API}/business/alerts", params={"district_id": 2}).json()
    assert district_2 == []  # the only row there is MONITOR, filtered out

    unfiltered = client.get(f"{API}/business/alerts").json()
