import { createFileRoute, Link } from "@tanstack/react-router";
import { useMemo, useState } from "react";
import { CloudRain, Trophy } from "lucide-react";
import { getScenarioResult } from "@/lib/khetiService";
import { DemoDataBadge, Disclaimer, Pill } from "@/components/kheti/primitives";

export const Route = createFileRoute("/whatif")({
  head: () => ({
    meta: [
      { title: "Weather what-if scenario | KhetiSetu" },
      {
        name: "description",
        content:
          "Move the rainfall slider and see how crop opportunity scores and the safest recommendation change.",
      },
      { property: "og:title", content: "Weather what-if scenario | KhetiSetu" },
      { property: "og:description", content: "Simulate rainfall changes on crop suitability." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: WhatIf,
});

function WhatIf() {
  const [delta, setDelta] = useState(0);
  const result = useMemo(() => getScenarioResult(delta), [delta]);

  return (
    <div className="mx-auto max-w-4xl px-4 py-8 md:px-6 md:py-12">
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
        <div className="min-w-0">
          <h1 className="text-2xl font-extrabold text-foreground md:text-3xl">What if the weather changes?</h1>
          <p className="mt-2 text-muted-foreground">
            Adjust expected rainfall and see how crop opportunity scores respond.
          </p>
        </div>
        <DemoDataBadge label="Demo scenario" />
      </div>

      <section className="surface-card mt-6 p-5 md:p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <span className="inline-flex items-center gap-2 text-sm font-semibold text-foreground">
            <CloudRain className="h-4 w-4 text-info" aria-hidden /> Expected rainfall
          </span>
          <span className="text-2xl font-extrabold text-foreground">{100 + delta}%</span>
        </div>

        <label htmlFor="rainfall" className="mt-4 block text-sm font-semibold text-foreground">
          Rainfall change vs normal
        </label>
        <input
          id="rainfall"
          type="range"
          min={-30}
          max={30}
          step={5}
          value={delta}
          onChange={(e) => setDelta(Number(e.target.value))}
          aria-valuetext={`${delta > 0 ? "+" : ""}${delta} percent rainfall`}
          className="mt-3 h-2 w-full cursor-pointer appearance-none rounded-full bg-muted accent-[var(--color-primary)]"
        />
        <div className="mt-2 flex justify-between text-xs font-medium text-muted-foreground">
          <span>-30%</span>
          <span>Normal</span>
          <span>+30%</span>
        </div>

        <div className="mt-3 flex flex-wrap gap-2">
          {[-30, -15, 0, 15, 30].map((v) => (
            <button
              key={v}
              type="button"
              onClick={() => setDelta(v)}
              className={`rounded-lg border px-3 py-2 text-sm font-semibold transition-colors ${
                delta === v ? "border-primary bg-primary/10 text-primary" : "border-border bg-card text-muted-foreground hover:bg-muted"
              }`}
            >
              {v > 0 ? `+${v}%` : v === 0 ? "Normal" : `${v}%`}
            </button>
          ))}
        </div>
      </section>

      <section className="mt-6 space-y-3" aria-live="polite">
        <h2 className="text-lg font-bold text-foreground">Opportunity scores in this scenario</h2>
        {result.crops.map((c, i) => (
          <article key={c.id} className="surface-card p-4">
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex h-6 min-w-6 items-center justify-center rounded-full bg-muted px-2 text-xs font-bold text-muted-foreground">
                    #{i + 1}
                  </span>
                  <h3 className="truncate text-lg font-bold text-foreground">{c.crop}</h3>
                  {i === 0 && result.changed && <Pill tone="harvest" icon={Trophy}>New safer option</Pill>}
                </div>
                <div className="mt-2 h-2 w-full overflow-hidden rounded-full bg-muted" role="presentation">
                  <div
                    className="h-full rounded-full bg-primary transition-[width] duration-500 ease-out"
                    style={{ width: `${c.score}%` }}
                  />
                </div>
              </div>
              <div className="text-right">
                <p className="text-2xl font-extrabold tabular-nums text-foreground transition-all duration-300">
                  {c.score}%
                </p>
                <p
                  className={`text-xs font-semibold ${c.delta > 0 ? "text-success" : c.delta < 0 ? "text-destructive" : "text-muted-foreground"}`}
                >
                  {c.delta === 0 ? "No change" : `${c.delta > 0 ? "▲ +" : "▼ "}${c.delta} vs baseline (${c.baseScore}%)`}
                </p>
              </div>
            </div>
          </article>
        ))}
      </section>

      <section className="surface-card mt-6 border-harvest/40 bg-harvest-soft/15 p-5">
        <h2 className="flex items-center gap-2 text-base font-bold text-foreground">
          <Trophy className="h-5 w-5 text-harvest" aria-hidden />
          {result.changed ? "Recommendation changed" : "Recommendation unchanged"}
        </h2>
        <p className="mt-2 text-sm leading-relaxed text-foreground">
          {result.changed ? `Safer option under this scenario: ${result.leader.crop}. ` : ""}
          {result.explanation}
        </p>
        <Link
          to="/crop/$cropId"
          params={{ cropId: result.leader.id }}
          className="mt-4 inline-flex w-full items-center justify-center rounded-lg bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground hover:bg-primary-dark sm:w-auto"
        >
          View {result.leader.crop} crop plan
        </Link>
      </section>

      <div className="mt-6">
        <Disclaimer>
          This is a simulated scenario using demo sensitivity values. Real weather impact varies by soil,
          variety and field management.
        </Disclaimer>
      </div>
    </div>
  );
}
